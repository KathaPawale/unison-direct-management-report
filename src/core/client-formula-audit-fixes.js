...
(function(){
  ...
  const metricLines=sm=>({
    income:findLine(sm,[/^total for income$/, /^total income$/, /^total revenue$/, /^revenue$/, /^sales$/, /total.*(income|revenue|sales)/]),
    gross:findLine(sm,[/^gross profit$/]),
    expenses:findLine(sm,[/^total for expenses$/, /^total expenses$/, /^total operating expenses$/, /total.*expenses/]),
    net:findLine(sm,[/^net income$/, /^net profit$/, /^profit for the period$/])
  });
  ...
  function deriveComparative(md){
    const candidates=[md.roles?.plComparative,md.roles?.plMonthly,md.roles?.pl].filter(Boolean).map(name=>md.sheetModels?.[name]).filter(Boolean);
    let chosen=null,currentCols=[],priorCols=[];
    for(const sm of candidates){
      const pcs=periodCols(sm), months=pcs.map(c=>({c,y:year(c.label),m:monthNo(c.label)})).filter(x=>x.y&&x.m);
      const monthYears=[...new Set(months.map(x=>x.y))].sort((a,b)=>b-a);
      if(monthYears.length>=2){
        const cy=monthYears[0],py=monthYears[1],cm=months.filter(x=>x.y===cy),available=new Set(cm.map(x=>x.m));
        const pm=months.filter(x=>x.y===py&&available.has(x.m));
        if(cm.length&&pm.length){chosen=sm;currentCols=cm.map(x=>x.c);priorCols=pm.map(x=>x.c);break;}
      }
      const annual=pcs.map(c=>({c,y:year(c.label)})).filter(x=>x.y&&!monthNo(x.c.label));
      const ys=[...new Set(annual.map(x=>x.y))].sort((a,b)=>b-a);
      if(ys.length>=2){chosen=sm;currentCols=annual.filter(x=>x.y===ys[0]).map(x=>x.c).slice(0,1);priorCols=annual.filter(x=>x.y===ys[1]).map(x=>x.c).slice(0,1);break;}
      const cur=(sm.cols||[]).find(c=>c.type==='current'),pri=(sm.cols||[]).find(c=>c.type==='prior');
      const cy=year(cur?.label),py=year(pri?.label);
      if(cur&&pri&&cur.idx!==pri.idx&&((cy&&py&&cy!==py)||/prior|previous|last year/i.test(String(pri.label||'')))){chosen=sm;currentCols=[cur];priorCols=[pri];break;}
    }
    if(chosen&&currentCols.length&&priorCols.length){
      const cur=readMetricSet(chosen,currentCols),pri=readMetricSet(chosen,priorCols);
      md.metrics={...(md.metrics||{}),...cur};
      md.prior={...(md.prior||{}),...pri};
      md.comparativeAudit={sheet:chosen.name,currentColumns:currentCols.map(c=>c.label),priorColumns:priorCols.map(c=>c.label)};
      return {sm:chosen,currentCols,priorCols};
    }
    md.prior={income:null,gross:null,expenses:null,net:null};
    return null;
  }

  function directExpenseRows(sm){
    const lines=sm?.lines||[], opener=lines.findIndex(l=>/^(expenses|operating expenses)$/.test(norm(l.label))), totalIdx=lines.findIndex(l=>/^total (for )?(operating )?expenses$/.test(norm(l.label)));
    if(opener<0)return [];
    const end=totalIdx>opener?totalIdx:lines.length;
    const out=[];let skipTo=-1;
    for(let i=opener+1;i<end;i++){
      const l=lines[i];if(l.r<=skipTo)continue;
      if(l.totalIdx!=null&&l.totalIdx>i&&l.totalIdx<end){const tl=lines[l.totalIdx];out.push({label:l.label,line:tl});skipTo=tl.r;}
      else if(l.kind==='account')out.push({label:l.label,line:l});
    }
    return out;
  }

  function fixExpenses(md,ctx){
    const sm=ctx?.sm || (md.roles?.plMonthly&&md.sheetModels?.[md.roles.plMonthly]) || (md.roles?.plComparative&&md.sheetModels?.[md.roles.plComparative]) || (md.roles?.pl&&md.sheetModels?.[md.roles.pl]);
    if(!sm)return;
    let cols=ctx?.currentCols||[];
    if(!cols.length){const c=col(sm);if(c!=null)cols=[{idx:c,label:'current'}];}
    const L=metricLines(sm),total=L.expenses?Math.abs(cols.reduce((s,c)=>s+value(sm,L.expenses,c.idx),0)):Math.abs(n(md.metrics?.expenses));
    const rows=directExpenseRows(sm);
    if(rows.length&&total){
      md.expenseGroups=rows.map(r=>{const v=cols.reduce((s,c)=>s+value(sm,r.line,c.idx),0);return {label:r.label,value:v,pct:cleanPct(Math.abs(v)/total*100)};}).filter(x=>Math.abs(x.value)>0.004).sort((a,b)=>Math.abs(b.value)-Math.abs(a.value));
  ...
  function apply(md){
    if(!md)return md;
    const comparative=deriveComparative(md);
    fixExpenses(md,comparative);
    ...
    if(Array.isArray(md.months)){
      const real=md.months.map((m,i)=>({m,i})).filter(x=>monthNo(x.m?.short||x.m?.label));
      if(real.length&&real.length!==md.months.length){md.months=real.map(x=>x.m);if(Array.isArray(md.monthlyRevenue))md.monthlyRevenue=real.map(x=>n(md.monthlyRevenue[x.i]));if(Array.isArray(md.monthlyNet))md.monthlyNet=real.map(x=>n(md.monthlyNet[x.i]));if(Array.isArray(md.monthlyExpenses))md.monthlyExpenses=real.map(x=>n(md.monthlyExpenses[x.i]));}
    }
    return md;
  }
