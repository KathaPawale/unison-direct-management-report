 ...
 * Only years with an ACTUAL amount column are included. Latest real year is
 * current and is compared with every earlier real year. Missing years are not invented.
 */
'use strict';
(function(){
  ...
  function discoverYearColumns(rows,sm){
    const blocked=blockedCols(sm),keyRows=metricRows(rows),candidates=new Map();
    for(const r of (rows||[]).slice(0,60))for(let c=1;c<(r||[]).length;c++){if(blocked.has(c))continue;const ys=yearsIn(r[c]);if(ys.length!==1)continue;if(keyRows.some(kr=>toNum(kr?.[c])!=null)&&!candidates.has(ys[0]))candidates.set(ys[0],c);}
    for(const c of sm?.cols||[]){if(blocked.has(c.idx))continue;const ys=yearsIn(c.label||c.name);if(ys.length!==1)continue;if(keyRows.some(kr=>toNum(kr?.[c.idx])!=null)&&!candidates.has(ys[0]))candidates.set(ys[0],c.idx);}
  ...
  function build(){
    const md=state.model;if(!md)return null;
