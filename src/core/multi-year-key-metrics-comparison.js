 ...
 * Only years with an ACTUAL amount column are included. A year appearing only
 * in a report title is ignored. Latest real year is current and is compared
 * against every earlier real year. No missing year is invented.
 */
'use strict';
(function(){
  ...
  const toNum=v=>{
    ...
    const neg=/^\(.*\)$/.test(s); s=s.replace(/[$,()\s]/g,'');
  ...
  function discoverYearColumns(rows,sm){
    ...
    /* A year is accepted ONLY when that same column contains at least one
       recognized financial metric amount. This deliberately ignores title rows
       such as "For the Years Ended 2025, 2024 and 2023" when 2023 has no column. */
    for(const r of (rows||[]).slice(0,60)){
      for(let c=1;c<(r||[]).length;c++){
        if(blocked.has(c)) continue;
        const ys=yearsIn(r[c]); if(ys.length!==1) continue;
        if(keyRows.some(kr=>toNum(kr?.[c])!=null) && !candidates.has(ys[0])) candidates.set(ys[0],c);
      }
    }
    for(const c of sm?.cols||[]){
      if(blocked.has(c.idx)) continue;
      const ys=yearsIn(c.label||c.name); if(ys.length!==1) continue;
      if(keyRows.some(kr=>toNum(kr?.[c.idx])!=null) && !candidates.has(ys[0])) candidates.set(ys[0],c.idx);
    }
  ...
  function build(){
    ...
    /* Union contains only actual amount-column years discovered in statements. */
  ...
  function syncLatestToDashboard(d){
    const md=window.state?.model;if(!md||!d)return;
    const rev=metric(d,'revenue'),net=metric(d,'netIncome'),ar=metric(d,'ar'),ap=metric(d,'ap');
    md.metrics=md.metrics||{};md.prior=md.prior||{};
    if(rev?.current!=null)md.metrics.income=rev.current;if(net?.current!=null)md.metrics.net=net.current;if(ar?.current!=null)md.metrics.ar=ar.current;if(ap?.current!=null)md.metrics.ap=ap.current;
    const py=d.previousYears[0];
    if(py!=null){if(rev?.values?.[py]!=null)md.prior.income=rev.values[py];if(net?.values?.[py]!=null)md.prior.net=net.values[py];if(ar?.values?.[py]!=null)md.prior.ar=ar.values[py];if(ap?.values?.[py]!=null)md.prior.ap=ap.values[py];}
    md.currentYear=d.latestYear;md.comparisonYears=d.previousYears.slice();
  }

  function table(d){
    if(!d||d.years.length<2)return '';
    const heads=d.years.map((y,i)=>`<th>${y}${i===0?' (Latest / Current)':''}</th>`).join('');
    const body=d.comparisons.map(r=>`<tr><td class="lbl"><b>${esc(r.label)}</b></td>${d.years.map(y=>`<td class="val">${esc(fmt(r.values[y]))}</td>`).join('')}<td class="val year-variance">${r.previous.map(x=>`<div><b>${d.latestYear} vs ${x.year}:</b> ${esc(fmt(x.variance))} (${esc(fmtPct(x.variancePct))})</div>`).join('')||'—'}</td></tr>`).join('');
    return `<div class="multi-year-key-comparison"><h3>Year-wise Key Financial Comparison</h3><div class="chart-sub">Latest/current year: <b>${d.latestYear}</b>. Comparing only actual years found in the workbook: <b>${d.years.join(', ')}</b>.</div><div class="report-table-wrap"><table class="report-table multi-year-table"><thead><tr><th>Metric</th>${heads}<th>Variance</th></tr></thead><tbody>${body}</tbody></table></div></div>`;
  }
