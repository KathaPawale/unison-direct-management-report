...
const dbState = { config: null, reportId: null, ready: false, saving: false };
...
async function initSupabase(){
  try {
    ...
    dbState.config = c; dbState.ready = true; return true;
...
async function sbRequest(table, { method='GET', body=null, query='', prefer='' }={}){
  ...
  const headers = { apikey: dbState.config.publishableKey, Authorization:'Bearer '+dbState.config.publishableKey, 'Content-Type':'application/json' };
...
function metricRows(reportId){
  ...
  return [
    ['revenue','Revenue / Income',m.income], ['gross_profit','Gross Profit',m.gross],
    ['net_income','Net Income',m.net], ['cash_bank','Cash / Bank',m.bank],
    ['accounts_receivable','A/R Total',m.ar], ['accounts_payable','A/P Total',m.ap],
    ['expenses','Total Expenses',m.expenses], ['assets','Total Assets',m.assets],
    ['liabilities','Total Liabilities',m.liabilities], ['equity','Total Equity',m.equity]
  ].filter(x => x[2] !== undefined && x[2] !== null).map(x => ({
...
async function saveReportToSupabase(){
  if (!hasData() || dbState.saving) return dbState.reportId;
  dbState.saving = true;
  try {
    if (!dbState.ready) await initSupabase();
    if (!dbState.ready) throw new Error('Supabase connection is unavailable');
    const created = await sbRequest('reports', { method:'POST', body:{
      client_name:state.client || 'Client', source_filename:state.fileName || '', period_label:state.period || '', status:'processed', notes:state.notes || ''
    }, prefer:'return=representation' });
    if (!created?.[0]?.id) throw new Error('Report was not created');
    dbState.reportId = created[0].id;
    const worksheets = Object.entries(state.sheets).map(([name, rows], i) => ({ report_id:dbState.reportId, sheet_name:name, sheet_order:i, data:rows }));
    if (worksheets.length) await sbRequest('worksheets', { method:'POST', body:worksheets });
...
async function logManualEdit(sheetName,rowIndex,columnIndex,oldValue,newValue){
  if (!dbState.reportId || !dbState.ready) return;
  try { await sbRequest('manual_edits',{method:'POST',body:{report_id:dbState.reportId,sheet_name:sheetName,row_index:rowIndex,column_index:columnIndex,original_value:String(oldValue??''),edited_value:String(newValue??'')}}); }
  catch(e){ console.warn('Could not log manual edit:',e); }
}

async function logReportExport(exportType,fileName){
  if (!dbState.reportId || !dbState.ready) return;
  try { await sbRequest('report_exports',{method:'POST',body:{report_id:dbState.reportId,export_type:exportType,file_name:fileName||''}}); }
  catch(e){ console.warn('Could not log report export:',e); }
}

/* Hooks keep the existing UI/processing code unchanged. */
document.addEventListener('DOMContentLoaded', () => {
  initSupabase();
  const processBtn = document.getElementById('processBtn');
  if (processBtn) processBtn.addEventListener('click', () => {
    const expected = document.getElementById('fileInput')?.files?.[0]?.name;
    if (!expected) return;
    dbState.reportId = null;
    let tries = 0;
    const wait = setInterval(async () => {
      tries++;
      if (state.fileName === expected && state.model && hasData()) {
        clearInterval(wait);
        try { await saveReportToSupabase(); toast('Processed and saved to Supabase'); }
        catch(e){ console.error(e); toast('Workbook processed, but database save failed: '+(e.message||e)); }
      } else if (tries > 100) clearInterval(wait);
    }, 100);
  });

  const editor = document.getElementById('editorTable');
  if (editor) editor.addEventListener('change', e => {
    const inp = e.target.closest('input[data-r][data-c]');
    if (!inp) return;
    logManualEdit(state.active, Number(inp.dataset.r), Number(inp.dataset.c), inp.dataset.old ?? '', inp.value);
  });
